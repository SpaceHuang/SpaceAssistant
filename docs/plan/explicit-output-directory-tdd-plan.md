...
- [x] 为提取器写 RED 测试：紧邻“文件、目录、保存为”等词的单段名称被识别。
  - RED（2026-07-18）：初始提取器仅处理引号路径。
- [x] 实现保守关键词邻近规则，使测试通过。
  - GREEN（2026-07-18）：仅在输出关键词后提取单段名称，3 条 evidence 测试与 Electron 编译通过。
- [x] 为提取器写 RED 测试：参考、读取、检查路径标为 `referenced-input`，不得成为 output target。
...
- [x] 为 resolver 写 RED 测试：模型伪造 user source 或 evidenceId 被拒绝。
...
- [x] 为 resolver 写 RED 测试：有未消费强输出证据的新 package/scratch 写入返回 `ARTIFACT_EXPLICIT_PATH_UNRESOLVED`。
  - RED（2026-07-18）：显式路径 resolution guard 尚不存在，测试模块无法导入。
- [x] 实现未解析强证据阻断逻辑，使测试通过。
  - GREEN（2026-07-18）：新 package/scratch 在存在未消费 output evidence 时均会稳定拒绝；3 条专用测试与 Electron 编译通过。
- [x] 为 `resolveOutputPathKind()` 写 RED 测试：已存在路径以 lstat 类型为准，显式声明冲突返回 `ARTIFACT_PATH_TYPE_CONFLICT`。
...
- [x] 为路径类型写 RED 测试：无扩展名文件、带点目录不按名称猜测类型；仅真正二义时请求 decision。
...
- [x] 为 `ArtifactPathLeaseRegistry` 写 RED 测试：use 可共享，write/delete 排他。
...
- [x] 为 package primary 写 RED 测试：未指定位置发出 output-location decision，不创建临时文件。

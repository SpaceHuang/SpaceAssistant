...
|---|---|
| 服务名称 | 必填，唯一，1–64 字符。 |
| 启用此服务 | 新建时默认关闭；最近一次测试成功且发现至少一个工具后即可打开，开启时自动启用全部已发现工具，用户可随后在编辑中调整白名单。测试后失联、认证失效或工具清单为空时自动关闭。 |
...
成功发现工具后，在服务卡片中显示工具列表：原始工具名、描述、风险提示、启用开关和「调用前确认」策略。默认所有工具为**未授权**；开启服务时自动启用全部已发现工具，用户可随后在编辑中调整白名单，服务才会出现在 Agent 可用列表。

| 配置项 | 默认值 | 规则 |
|---|---|---|
| 工具启用 | 关闭 | 按 Server + 原始工具名保存白名单；服务端新增工具不会自动启用。 |
| 调用前确认 | 只读工具可自动调用 | Server 声明 `readOnlyHint: true` 且未声明 `destructiveHint` 的工具免确认，其余每次展示确认卡片；可改为「始终确认」。 |

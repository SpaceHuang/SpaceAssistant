...
这些确认机制设计于「可能有外部攻击者发送指令」的旧威胁模型下。当前工作树已实现飞书 P2P + owner 校验，以及微信绑定发送者强制；外部身份冒用面已显著下降。逐次确认对「防未授权」的收益因此下降，而流程成本不变。本需求在这层已落地地基上降低摩擦，并保留灾难性硬拦截与可替代的访问控制开关。v1.6 要求补齐一次性配对码，以消除现有绑定窗口的抢绑缺口。

### 1.2 威胁模型转变

**当前安全基线与仍需关闭的缺口：**

1. **已实现：**飞书仅接收 p2p，并仅允许已绑定 owner 驱动 Agent。当前绑定窗口仍采用首条 p2p 绑定；v1.6 将其列为 P0 缺口，必须升级为 §2.1.2 的一次性配对码。  
2. **已实现：**微信仅允许已绑定发送者驱动 Agent；空 allowlist 或不匹配发送者 fail closed（见 §2.1.3，**不以「天然一对一」跳过验收**）。  
3. **范围外：**不考虑设备被盗（A2）的应用层防护（见 NG1）。  
4. **残余风险：**账号劫持 / Bot 凭据泄露（A3）仍可能存在，但不靠逐次确认防御；确认在凭据被控时同样可能被绕过。
...
| ID | 目标 | 优先级 |
|----|------|--------|
| G1 | 在既有 owner 校验 + p2p only 基线上引入抗抢跑一次性配对 | P0 |
...
| G4 | 在既有远程 IM「确认并信任」协议上，将 Shell 信任存储升级为结构化范围 | P0 |
| G5 | 废除 `remoteConfirmPolicy` 行为消费；确认/拒绝由明确开关表驱动；保留出站硬拒绝能力 | P0 |
| G6 | 保持飞书 owner / 微信绑定用户的发送者强制，并完成 legacy 群聊/前缀配置迁移 | P0 |
...
当前 `feishuInboundParser.ts` 的 `shouldAcceptInbound` 已默认拒绝 `chatType === 'group'`，仅接受私聊；后续重构必须保持该行为。
...
微信 Bot 多为扫码绑定后的会话。当前 `weChatCommandRouter.ts` 已在 allowlist 为空或发送者不匹配时拒绝入站，`weChatIpc.ts` 会从扫码绑定结果回填 allowlist。本需求要求保持并补齐迁移回归：

| 项 | 当前工作树 / 目标 |
|----|------|
| 远程监听启用时 | 已从扫码绑定结果回填；必须持续存在有效绑定发送者（allowlist / 既有绑定用户字段） |
| 空 allowlist | 已 fail closed；保持拒绝并提示重新绑定 |
| 非绑定发送者 | 已拒绝入站并记录 `wechat.reject.non_owner`；保持该行为 |

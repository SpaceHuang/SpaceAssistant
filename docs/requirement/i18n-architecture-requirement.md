...
> **当前阶段：** 第一期「基础设施」已完成；第二期「核心模块迁移」待启动
...
| 第二期 | `feat/i18n`（续） | common / chat / config 全量 + errors 主进程适配 | ⏳ 待启动 | 见 §8 第二期 |
...
| 错误码骨架 | `src/shared/errorCodes.ts`、`src/renderer/utils/errorTranslator.ts` |
| 翻译资源 | `resources/zh-CN/`、`resources/en-US/` 下 **common**、**config**、**errors**（共 6 个 JSON） |
...
| US-03 | UI 文案全覆盖 | 🔴 未开始 | 仅设置页壳层与通用 Tab 部分字段已迁移（见下） |
| US-04 | 主进程文案覆盖 | 🟡 骨架完成 | `translateError` + `errors.json` 就绪；`appIpc.ts` 等主进程错误仍硬编码 |
| FR-01 | i18next 初始化 | ✅ 已完成 | — |
...
| FR-05 | 文案迁移策略 | 🟡 进行中 | **已迁移：** 设置页导航标签、页眉/页脚按钮、通用 Tab 工作目录与语言控件；**未迁移：** 活动栏、聊天、文件树等（§8 第二/三期） |
| FR-06 | 日期/时间格式化 | 🔴 未开始 | `formatChatTimestamp` 仍用系统 locale |
| FR-07 | 主进程错误码 | 🟡 骨架完成 | 5 个错误码 + 双语 `errors.json`；主进程 throw 改造留第二期 |
| FR-08 | Ant Design 国际化 | ✅ 已完成 | `ConfigProvider` 随 `i18next.language` 切换 |
| FR-09 | shared 目录文案 | 🔴 未开始 | `appMeta.ts` 等仍硬编码 |

#### 第一期已迁移 UI 范围（验证切换链路）

- 设置页：侧栏分类标签（通用 / 模型 / 技能 / Wiki / 飞书）、「工具」分组标题、页眉标题、返回/取消/应用/保存并返回、未保存提示
- 通用 Tab：工作目录标签与选择目录、界面语言 Select 及提示文案
- **仍硬编码中文：** 通用 Tab「并行会话上限」等表单项、模型/工具/技能/Wiki/飞书各 Tab、活动栏、聊天区、App 壳层消息等（`i18n:check` 扫描约 900+ 处 warn，属预期）

#### 已知限制与后续动作

1. **`i18n:check` 硬编码扫描**：当前对 renderer 内中文仅 **warn**，不导致脚本失败；第四期全量迁移后再启用 `--strict-hardcoded` 或改为 fail。
2. **命名空间懒加载**（FR-01）：尚未实现，首期资源体积小，全量打包。
3. **既有测试失败**：全量 `npm test` 仍有 4 个失败（`ThinkingBlock`×3、`ShellSettingsTab` 超时×1），与 i18n 无关；合并前需单独处理或确认基线。
4. **第二期建议顺序**：`errors` 主进程改造 → `chat` → `config` 全量 → `common` 全项目通用文案。
...
### 第二期：核心模块迁移（预计 5–8 天）— ⏳ 待启动

| # | 任务 | 状态 |
|---|------|------|
| 1 | `common` 命名空间 — 全项目通用文案替换 | ⏳ |
| 2 | `chat` 命名空间 — 聊天界面全部文案 | ⏳ |
| 3 | `config` 命名空间 — 设置面板全部文案 | 🟡（通用 Tab 部分 key 已用，其余 Tab 未迁移） |
| 4 | `errors` 命名空间 — 错误消息体系 + 主进程错误码适配 | 🟡（资源与 `translateError` 就绪，主进程未改） |
...
| `formatChatTimestamp` | 验证日期格式化在 zh-CN / en-US locale 下的输出格式 | P1 | ⏳ 待 FR-06 |
| `buildBrowserSetupGuideContent` | 验证工厂函数接收翻译函数后的输出正确性 | P1 | ⏳ 待 FR-09 |
| 语言检测逻辑 | 验证 `navigator.language` → locale 映射规则（zh 开头 → zh-CN，其他 → en-US） | P1 | ✅ `detectLocale.test.ts` |
| `changeLanguage` 性能 | `changeLanguage` 耗时 < 200ms | P1 | ✅ `changeLanguage.test.ts` |
| `configModalSnapshot` | locale 纳入脏检查 | P1 | ✅ `configModalSnapshot.test.ts` |
...
> 第一期完成后状态（2026-06-03，`feat/i18n`）

- [x] 应用首次启动能自动检测系统语言（主进程 `readAppLocale` + 渲染进程检测；建议手工 E2E 再确认）
- [x] 设置面板可切换语言，切换后即时生效
- [ ] 中文界面所有文案正确显示（与迁移前一致）— 仅设置页已迁移部分满足；全局未验收
- [ ] 英文界面所有文案为正确英文（无中文残留）— 全局未验收；设置页已迁移部分可切换验证
- [x] Ant Design 组件文案跟随应用语言
- [ ] 日期时间格式随语言变化
- [x] 语言选择持久化（重启后保持）— `AppConfig.locale` + `sa_locale`
- [ ] 所有组件在两种语言下无布局溢出
- [x] 错误消息在两种语言下正确显示 — 仅已定义错误码（5 个）经 `translateError` 验证
- [x] `npm run i18n:check` 通过（key 对齐、JSON 合法；硬编码为 warn）
- [ ] 现有测试全部通过 — 877/881（4 个既有失败，见 §1.3）
- [x] 新增 i18n 相关单元测试覆盖 — 13 用例，5 个测试文件

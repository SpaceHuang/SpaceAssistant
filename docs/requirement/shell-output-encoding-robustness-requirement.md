...
**版本：** 1.1
**日期：** 2026-09-12
**状态：** 已按评审 v1.0 修订（B1/B2 已解决）；D1/D2/D3/D5 已由用户确认、D4/D6/D7 待定（§14.1）；待抽查复评
...
3. 产出决策记录：已确认项（D1/D2/D3/D5）的**实测复核结果**与未决项（D4/D6/D7）的建议（§14.1）。
...
| D4 | 是否新增用户级 `shellConfig.outputEncoding` 覆盖 | 现在做 / 延后 | **延后到 Phase 3**：先靠内置 profile + OEMCP 探测覆盖，避免引入配置迁移面 | 评审 |
...
> **来源与效力（M2 修订）**：下表 D1/D2/D3/D5 的结论来自**需求方（用户）在本会话中的逐条确认**，不是评审人意见；D4/D6/D7 仍是本文档的**建议**。因此本节**不等于**「文档已通过评审定稿」——文档状态见文首元信息。Phase 0 对已确认项只做**实测复核**（跑现有测试、确认无隐藏依赖），不再「定稿」。
...
| D4 / D6 / D7 | 维持建议（D4 延后到 Phase 3、D6 终端保持 raw 直达 xterm、D7 写进 shell 使用提示） | 本轮未单独评审 |

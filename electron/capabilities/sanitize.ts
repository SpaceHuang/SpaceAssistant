const CREDENTIAL_KEY_PATTERN =
  /^(api[_-]?key|password|passwd|secret|token|access[_-]?token|refresh[_-]?token|authorization|x-api-key|credentials?|private[_-]?key|client[_-]?secret|header[_-]?value|header[_-]?name)$/i
...
export function sanitizeCapabilityResult(value: unknown): unknown {
  ...
  const walk = (v: unknown, key?: string): unknown => {
    if (key && (CREDENTIAL_KEY_PATTERN.test(key) || key.startsWith('env:'))) {
      return Boolean(v)
    }

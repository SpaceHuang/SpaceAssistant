...
interface FeishuProcessedStoreData {
  entries: Array<{ messageId: string; processedAt: number }>
}

export class FeishuProcessedStore {
  private data: FeishuProcessedStoreData = { entries: [] }
  private filePath: string
  private loaded = false

  constructor(userDataDir: string) {
    this.filePath = path.join(userDataDir, 'feishu-processed-messages.json')
  }
  ...
  private async ensureLoaded(): Promise<void> {
    ...
    try {
      ...
      this.data = JSON.parse(raw) as FeishuProcessedStoreData
    } catch {
      this.data = { entries: [] }
    }
    this.purgeExpired()

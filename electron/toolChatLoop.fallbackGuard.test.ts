...
describe('四维判定第④维（评审 v3 B2）：中止守卫', () => {
  it('主通道等待期间 chat aborted（环境失败恰好落在可回退格）→ 不回退、不弹卡', async () => {
    installChannelSequence([agentFailure('unavailable')], [], {
      // 第一跳请求内置位 aborted 再返回 unavailable——模拟「结算值命中③、由第④维中止守卫拦截」：
      // Skill v2.1 后用户停止以 cancelled 结算（③ 挡），租约恢复失败路径仍会以 unavailable 结算
      first: (_entry, _req) => {

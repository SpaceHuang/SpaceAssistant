...
describe('ContextMeter service', () => {
  it('replays events on every measure and does not use a process-only anchor', () => {
    const meter = new ContextMeter(() => [
      ...
      { seq: 2, type: 'request_context', payload: { schemaVersion: 1, requestId: 'r', provider: 'p', model: 'm', contextWindow: { tokens: 100, source: 'config' }, budget: { estimatorVersion: 'v', serializationVersion: 's' } } },

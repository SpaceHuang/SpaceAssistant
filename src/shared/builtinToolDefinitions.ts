...
/**
 * 内置工具确认/风险元数据（P0 概念收敛）。
 *
 * 说明：为避免元数据字段泄漏到 Anthropic / OpenAI 兼容网关的 tools 载荷，元数据独立于
 * BUILTIN_TOOL_DEFINITIONS 维护，不直接挂在工具定义对象上。riskLevel 在 P0 与既有
 * builtinToolRiskLevel 完全一致（对外行为不变）；browser→medium、wechat 出站→low 是
 * 计划 §4 的演进目标，留待允许行为变化的阶段统一。
 */
